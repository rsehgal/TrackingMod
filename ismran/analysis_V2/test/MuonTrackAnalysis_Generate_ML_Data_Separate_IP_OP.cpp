...
int main(int argc, char *argv[])
{
  ...
  std::ofstream outfileInputFeatures("input_features.txt");
  std::ofstream outfileOutputFeatures("output_features.txt");
  for (unsigned int i = 0; i < smtVec.size(); i++) {
    ...
    if (smtVec[i]->HitInRequiredLayers()) {
      ...
      std::vector<lite_interface::ScintillatorBar_V2 *> vecOfScint = smtVec[i]->GetMuonTrack();
      for (unsigned int j = 0; j < vecOfScint.size(); j++) {
        outfileInputFeatures << vecOfScint[j]->GetLayerIndex() << "," << vecOfScint[j]->GetBarIndex() << ","
                             << vecOfScint[j]->GetLogQNearByQFar_ForSimulation() << ","
                             << vecOfScint[j]->GetDelTCorrected() << ","; 
      }

	outfileInputFeatures << std::endl;
      outfileOutputFeatures << formula_Exact->GetParameter(0) << "," << formula_Exact->GetParameter(1) << std::endl;
    }
  }
  outfileInputFeatures.close();
  outfileOutputFeatures.close();
  // outfile.close();
  // fApp->Run();
}
