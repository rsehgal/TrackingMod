...
Analyzer::Analyzer(std::string datafileName, const char *outputfileName)
    : Analyzer(datafileName, std::string(outputfileName))
{
}
...
// Analyzer::Analyzer(std::string datafileName,std::string outputfileName, bool simulation) :
// fDatafileName(datafileName), fOutputfileName(outputfileName){
Analyzer::Analyzer(std::string datafileName, std::string outputfileName)
    : fDatafileName(datafileName), fOutputfileName(outputfileName)
{
...
void Analyzer::CreateScintillatorVector(bool thresholdCheck, unsigned int numOfEv )
{
  ...
  for (unsigned long int i = 0; i < numOfEvToRead;) {
    ...
    ULong64_t tstampSmall = (vectorOfPairedTreeEntries[i]->tstamp < vectorOfPairedTreeEntries[i + 1]->tstamp)
                                ? vectorOfPairedTreeEntries[i]->tstamp
                                : vectorOfPairedTreeEntries[i + 1]->tstamp;
    ...
    scint = new ScintillatorBar_V2(
      ...
      // Before introducing vertical arrangement
       scint = new ScintillatorBar_V2(
         ...
         */
        /*
        scint->deltaTstampCorrected = scint->deltaTstamp -
        fCalib->GetCalibrationDataOf(scint->barIndex)->fDeltaTCorr*1000; scint->qlongMeanCorrected = scint->qlongMean +
        fCalib->GetCalibrationDataOf(scint->barIndex)->fEnergyCalibrationFactor;
...
void Analyzer::CreateScintillatorVector_FromSimulation()
{
  ...
  */
  ...
  for (Long64_t i = 0; i < nentries; i++) {
    ...
    if (!(i % 100000)) std::cout << "Processed : " << i << " events....." << std::endl;
  ...
  for (unsigned int i = 1; i < scintVecSize; i++) {
    ...
    if (fVecOfScintillatorBar[i]->GetQMeanCorrected() > qmeanCorrThreshold) {
      ...
      if (std::fabs(fVecOfScintillatorBar[i]->fTSmallTimeStamp - tStart) < 20000) {
        ...
        if (fVecOfScintillatorBar[i]->fTSmallTimeStamp < tStart) tStart = fVecOfScintillatorBar[i]->fTSmallTimeStamp;
      } else {
        ...
        */
            ...
            if (count < 4) {
              ...
              for (unsigned short k = 0; k < vec.size(); k++) {
                std::cout << "BarName : " << barNamesVector[k]
                          << " : Corrected DelT : " << (singleMuonTrack->GetMuonTrack())[k]->GetDelTCorrected()
                          << " : ";
                // vec[k]->Print();
              }
            }
        ...
        if (singleMuonTrack->size() == numOfLayers) hitInAllLayersCounter++;
