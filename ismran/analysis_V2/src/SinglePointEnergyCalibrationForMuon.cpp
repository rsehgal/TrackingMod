...
SinglePointEnergyCalibrationForMuon::SinglePointEnergyCalibrationForMuon(std::vector<lite_interface::ScintillatorBar_V2*> vecOfScintillatorBar){
...
void SinglePointEnergyCalibrationForMuon::CreateScintillatorVector(){
	...
	for(unsigned long int i = 0 ; i < fVecOfPairedTreeEntry.size(); ){
		...
		Double_t qmean = sqrt(fVecOfPairedTreeEntry[i]->qlong * fVecOfPairedTreeEntry[i+1]->qlong);
				ULong64_t tstampSmall = (fVecOfPairedTreeEntry[i]->tstamp < fVecOfPairedTreeEntry[i+1]->tstamp) ?
		...
		if((fVecOfPairedTreeEntry[i]->brch)/2 < (numOfLayers*numOfBarsInEachLayer)){
			...
			if((qmean > qstart) && (qmean < qend)){
				...
				lite_interface::ScintillatorBar_V2 *scint;
...
//				EstimateHitPosition(scint);
			...
			if((qmean > qstart) && (qmean < qend))
			{
							...
							if(fVecOfPairedTreeEntry[i]->brch < fVecOfPairedTreeEntry[i+1]->brch){
								...
								scint = new lite_interface::ScintillatorBar_V2(fVecOfPairedTreeEntry[i]->brch/2,fVecOfPairedTreeEntry[i]->qlong,
															   qmean, tstampSmall,(fVecOfPairedTreeEntry[i]->tstamp-fVecOfPairedTreeEntry[i+1]->tstamp));

							}else{
								scint = new lite_interface::ScintillatorBar_V2(fVecOfPairedTreeEntry[i+1]->brch/2,fVecOfPairedTreeEntry[i+1]->qlong,
															   qmean, tstampSmall,(fVecOfPairedTreeEntry[i+1]->tstamp-fVecOfPairedTreeEntry[i]->tstamp));
							}
							/*
							 * Doing DelT correction if within the energy range
							 */
...
void SinglePointEnergyCalibrationForMuon::FillHistograms(){
	for(unsigned long int i = 0 ; i < fVecOfScintillatorBar.size() ; i++){
		std::cout << "ScintillatorBAR : " << fVecOfScintillatorBar[i]->fBarIndex << std::endl;
		fhistogramsVec[fVecOfScintillatorBar[i]->fBarIndex]->FillHistogram(fVecOfScintillatorBar[i]);
	}
}
