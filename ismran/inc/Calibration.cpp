...
Calibration::Calibration(std::string fileName)
{
  ...
  for (unsigned int barIndex = 0; barIndex < numOfBars; barIndex++) {
    ...
    TF1 *paramertization_F =
        new TF1(Form("fzparam_%s", vecOfBarsNamess[barIndex].c_str()), temp_F->GetExpFormula(), -60.0, 60.0);
    ...
    for (unsigned short int sourceIndex = 0; sourceIndex < vecOfSourcePositions.size(); sourceIndex++) {
      vecOfDelTFormula.push_back((TF1 *)fp->Get(
          Form("fdelt_shift_Cs137_%s_%dcm", vecOfBarsNamess[barIndex].c_str(), vecOfSourcePositions[sourceIndex])));
    }
    // fVecOfCalibrationData.push_back(new CalibrationData( delTShift_F, temp_F
    // ,gr,vecOfDelTFormula,energyCalibrationFactor));
    /*
     * Also including energy calibration formula
     */
    fVecOfCalibrationData.push_back(
        new CalibrationData(delTShift_F, temp_F, gr, vecOfDelTFormula, enerCalibFormula, energyCalibrationFactor));
#if (0)
    if (barPhyNum <= 70)
      fVecOfCalibrationData.push_back(
          new CalibrationData(delTShift_F, temp_F, gr, vecOfDelTFormula, enerCalibGraph, energyCalibrationFactor));
    else {
      // fVecOfCalibrationData.push_back(new CalibrationData( delTShift_F, temp_F
      // ,gr,vecOfDelTFormula,enerCalibFormula,energyCalibrationFactor));
      /*
...
Calibration::Calibration(std::string fileName)
{
  ...
  for (unsigned int barIndex = 0; barIndex < numOfBars; barIndex++) {
    ...
    // std::cout << "Parameters of " << vecOfBarsNamess[barIndex] << " : " <<  enerCalibFormula->GetParameter(0) << " ,
    // " << enerCalibFormula->GetParameter(1) << std::endl; std::cout << "DelTparam : " << delTShift_F->GetParameter(1)
    // << std::endl;
