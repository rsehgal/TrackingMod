...
struct CalibrationData{
	...
	CalibrationData(TF1* delTCorr, TF1* parameterization, TGraph* parameterization_g, std::vector<TF1*> vecOfDelTFormula,TGraphErrors *calibGraph, float energyCalibrationFactor=1){
					fDelTCorr_F = delTCorr;
					fParameterization_F = parameterization;
					fDeltaTCorr = fDelTCorr_F->GetParameter(1);
					fEnergyCalibrationFactor = energyCalibrationFactor;
	...
	CalibrationData(TF1* delTCorr, TF1* parameterization, TGraph* parameterization_g, std::vector<TF1*> vecOfDelTFormula,TF1* energyCalibFormula, float energyCalibrationFactor=1){
						fDelTCorr_F = delTCorr;
						fParameterization_F = parameterization;
						fDeltaTCorr = fDelTCorr_F->GetParameter(1);
						fEnergyCalibrationFactor = energyCalibrationFactor;
