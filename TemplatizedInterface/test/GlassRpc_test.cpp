...
void Print(Detector *rpc1) {
  ...
  for (int i = 0; i < numOfPlanes; i++) {
    ...
    for (int j = 0; j < numOfChannels; j++) {
      // std::cout<<"ChannelId : " << rpc1->GetScintillatorPlaneVector()[i]->GetScintVector()[j]->GetScintId() <<
      // std::endl;
      std::cout << "Name : " << rpc1->GetScintillatorPlaneVector()[i]->GetScintVector()[j]->GetName() << std::endl;
      std::cout << "Location : ";
      rpc1->GetScintillatorPlaneVector()[i]->GetScintVector()[j]->GetPlacedLocation().Print();
    }
  }
}
...
int main() {
  ...
  Detector *rpc0 = new GlassRpc(3, "SecondGlassRpc", 90, -1);
  Detector *rpc1 = new GlassRpc(3, "SecondGlassRpc", 60, -1);
  Detector *rpc2 = new GlassRpc(3, "SecondGlassRpc", 30, 63);
  //
  Detector *rpc3 = new GlassRpc(4, "SecondGlassRpc", -30, -1);
  Detector *rpc4 = new GlassRpc(4, "SecondGlassRpc", -60, -1);
  Detector *rpc5 = new GlassRpc(4, "SecondGlassRpc", -90, -1);
